    ...
    def get_symbol(self, cfg, is_train=True):
        ...
        if cfg.network.ROIALIGN:
            ...
            roi_pool = mx.symbol.ROIPooling(name='roi_pool', data=roi_data, rois=rois,
                                            pooled_size=pooled_size,
                                            spatial_scale=1.0 / feat_stride)
        head = self.get_stage4(roi_pool, cfg)
        head = mx.sym.Pooling(name='ave_c5', data=head, pool_type='avg', global_pool=True, kernel=(7, 7))
        # cls_score/bbox_pred
        cls_score = mx.symbol.FullyConnected(name='cls_score', data=head, num_hidden=num_classes)
        bbox_pred = mx.symbol.FullyConnected(name='bbox_pred', data=head, num_hidden=num_reg_classes * 4)
